...
export default function CartPanel({
  ...
  return (
    <div className="w-full lg:w-96 bg-white/40 border border-white/50 rounded-2xl p-4 flex flex-col h-full overflow-hidden shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto py-2 min-h-0 select-none">
        {items.length === 0 ? (
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    <span className="text-xs font-bold text-foreground font-poppins truncate max-w-[200px]">
                      {index + 1}. {item.name}
                    </span>
                    <button
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>Rate: {formatCurrency(item.price, symbol)}</span>
                    <span className="font-semibold text-foreground">
                      Total: {formatCurrency(amount, symbol)}
                    </span>
                  </div>

                  {/* Quantity and Line Discount adjustments */}
                  <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-1 bg-white border rounded-lg p-0.5 shadow-inner">
                      <button
                        onClick={() => updateQty(item.cart_item_id, item.quantity - 1)}
                        className="p-1 hover:bg-muted rounded text-foreground active:scale-90 transition-transform"
                        disabled={loading}
                      >
                    ...
                    <div className="flex items-center gap-1.5 border rounded-lg p-0.5 bg-white shadow-inner">
                      <Tag className="w-3 h-3 text-muted-foreground ml-1" />
                      <input
                        type="number"
                        min="0"
                        max="100"
                        placeholder="0"
                        className="w-10 text-center text-xs border-none focus:outline-none p-0"
      ...
      <div className="border-t border-muted/30 pt-3 space-y-3">
        {/* Global discount & Parcel charges input fields */}
        <div className="grid grid-cols-2 gap-2">
          <div className="flex items-center gap-1.5 border rounded-xl p-2 bg-white/60 border-white/50 shadow-xs">
            ...
            <div className="flex-1 min-w-0">
              ...
              <input
                ...
                min="0"
                max="100"
                className="w-full text-xs font-semibold focus:outline-none border-none p-0 mt-0.5"
        ...
        <div className="space-y-1 bg-white/30 border border-white/40 p-2.5 rounded-xl text-xs">
          ...
          <div className="flex justify-between font-extrabold text-foreground text-sm pt-1.5 border-t border-muted/20">
            <span>Total</span>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          ...
          <Button
            ...
            className="col-span-1 text-xs h-10 font-bold bg-primary hover:bg-primary/90"
            disabled={loading || items.length === 0}
          >
            Complete Order
          </Button>
          ...
          <Button
            ...
            className="col-span-1 text-xs h-10 font-bold bg-secondary hover:bg-secondary/90 text-secondary-foreground"
            disabled={loading || items.length === 0}
          >
            Complete & Print
          </Button>
